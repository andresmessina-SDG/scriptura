...
# Quotes longer than this get a "more"/"less" toggle (previewed to a few
# lines) so a 1,000-word Augustine entry doesn't bury the others.
_PREVIEW_CHARS = 320
_PREVIEW_LINES = 4
...
class CatenaReader:
    ...
    def _rebuild(self):
        ...
        n = len(self._entries)
        if self._era_filter is None:
            self._header.set_text(
                f'{ref} · {n} voice{"s" if n != 1 else ""}'
                if n else ref)
        else:
    ...
    def _card(self, e):
        ...
        if e.get('source_title') or e.get('source_url'):
            ...
            if e.get('source_url'):
                link = Gtk.Button(icon_name='external-link-symbolic')
        ...
        quote = Gtk.Label(label=text, xalign=0, wrap=True, selectable=True)
        quote.set_margin_top(2)
        card.append(quote)

        if len(text) > _PREVIEW_CHARS:
            quote.set_lines(_PREVIEW_LINES)
            quote.set_ellipsize(Pango.EllipsizeMode.END)
            toggle = Gtk.Button(label='more')
            toggle.add_css_class('flat')
            toggle.set_halign(Gtk.Align.START)
            toggle.connect('clicked', self._toggle_more, quote)
            card.append(toggle)

        return card

    def _toggle_more(self, btn, quote):
        if btn.get_label() == 'more':
            quote.set_lines(-1)
            quote.set_ellipsize(Pango.EllipsizeMode.NONE)
            btn.set_label('less')
        else:
            quote.set_lines(_PREVIEW_LINES)
            quote.set_ellipsize(Pango.EllipsizeMode.END)
