...
# Same rationale as _DORE_WIDTH: the zoom dialog is 960px wide, and the
# museum scans here are huge (a 1500px Dürer woodcut rendered ~1–3 MB —
# dense hatching compresses badly). 960 keeps the set at Doré-like sizes.
_CURATED_WIDTH = 960


def _ingest_plate_file(conn, images_dir, limit, fetch_images,
                       toml_name, source_name, prefix):
    """Shared loader for the self-describing plate TOMLs (every plate
    carries its own kind/tradition/artist/license metadata). A plate is
    fetched from Commons via `file`, or from any direct `url` (e.g. the
    Utrecht IIIF server) — `url` plates must also carry `source_url`."""
    with open(os.path.join(_HERE, toml_name), 'rb') as f:
        plates = tomllib.load(f)['plate']
    if limit:
        plates = plates[:limit]
    rows = 0
    for p in plates:
        n = p['n']
        ext = os.path.splitext(p.get('file', ''))[1] or '.jpg'
        rel = f'images/{prefix}_{n}{ext}'
        size = None
        if fetch_images:
            dest = os.path.join(images_dir, f'{prefix}_{n}{ext}')
            url = p.get('url') or commons_filepath_url(p['file'],
                                                       _CURATED_WIDTH)
            try:
                size = fetch(url, dest)
            except Exception as e:  # noqa: BLE001 — report and skip one plate
                print(f'  ! {source_name} {n} fetch failed: {e}')
                continue
        v = p['verse']
        v_end = p.get('verse_end', v)
        ch_end = p.get('chapter_end', p['chapter'])
        ...
        conn.execute(
            ...
            'source_url, license, attribution, artist, year, iconclass) '
            'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            (p['kind'], p['tradition'], p['title'], None, p['book'],
             encode(p['chapter'], v), encode(ch_end, v_end),
             p.get('passage_label'), rel, size, source_name, src_url,
             p['license'], p['attribution'], p['artist'], p.get('year'),
             None))
        rows += 1
        print(f'  + {source_name} {n}  {p["book"]} {p["chapter"]}:{v}  '
              f'{p["title"]}')
    return rows


def ingest_curated(conn, images_dir, width, limit, fetch_images):
    """Curated masterwork cycles from tools/curated_plates.toml — whole-book
    sets (Dürer's Apocalypse for Revelation, Blake's Job, the Raphael
    Cartoons for Acts) plus future one-off gap-fill plates."""
    return _ingest_plate_file(conn, images_dir, limit, fetch_images,
                              'curated_plates.toml', 'curated', 'cur')


def ingest_psalter(conn, images_dir, width, limit, fetch_images):
    """The Utrecht Psalter psalm cycle from tools/psalter_plates.toml
    (generated by gen_psalter.py) — one illustrated manuscript page per
    psalm, fetched from Utrecht University's IIIF image server."""
    return _ingest_plate_file(conn, images_dir, limit, fetch_images,
                              'psalter_plates.toml', 'psalter', 'psa')
