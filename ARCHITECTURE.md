...
Internal architecture and design notes for anyone working on the codebase. For the user-facing  
   
 introduction, see [README.md.](README.md "README.md")  
**What we're building**  
...
Development workflow is iterative and incremental — small, verifiable  
   
 changes with frequent feedback. Behavior reports and error messages  
   
 drive the work; explanations stay plain and assume no GTK4 background.  
**Tech stack**  
...
 (vnum_start, vtext_start, vtext_end). vtext_start = vnum_start + len(str(N)) + 2 chars (the " N " prefix).  
...
- **Drop-cap on v1** — first letter rendered at size="200%" weight="bold" rise="-2000" via a regex that skips leading Pango span tags. Suppressed  
   
...
   
 version / type / copyright / license / about). _apply_module_change (name) takes a module name string and applies all the side effects;  
   
...
 controller.get_current_event().get_modifier_state() & Gdk.ModifierType.CONTROL_MASK and return False when Ctrl isn't held  
   
...
**M1–M3:** Two-pane app, navigation, module manager.  
   
 **M4:** Strong's lexicon — per-word tagging, definition view with cross-refs,  
   
 per-pane lexicon drawer with back-nav, sync toggles, module manager filters.  
   
 **M5:** Whoosh search — overlay panel, bar-chart distribution, search history,  
   
 verse flash on click. Per-pane inline search (Ctrl+F).  
   
 **M6:** Study Journal — master-detail layout (Gtk.Paned); sidebar has  
   
 SearchEntry + 2x2 filter grid (type/tag/module/book) + scrollable list.  
   
 Detail pane shows ref + module + color swatches + underline toggle +  
   
 tags entry + note TextView + Save / Go-to-verse buttons. Color/underline  
   
 are live-saves (in-place row strip update, no full reload). Notes/tags  
   
 have explicit Save. Tag chips on each row are clickable to filter. Tag  
   
 Manager dialog (header button) lists every tag with rename / delete  
   
 actions; rename auto-merges. annotations.get_tag_counts/rename_tag/ delete_tag back it.  
   
 **M7:** Cross-Reference Panel — slim single-row, TSK fallback when OpenBible  
   
 isn't downloaded.  
   
 **M8:** Reading Plans — 6 built-in plans, progress tracking, today highlight,  
   
 multi-passage day popover.  
**Open-data integration:** open_data.py for OpenBible cross-refs (340k),  
   
 OpenBible topics (700+ tags), Dodson Greek Lexicon. Module Manager "Open  
   
 Databases" tab. Note editor shows OpenBible topics as suggested-tag chips.  
**eBible.org backend:** ebible_bridge.py with SQLite catalog for modern  
   
 translations not available as SWORD modules (LEB, BSB, etc.).  
**Morphology:** Greek (Robinson via MorphGNT) and Hebrew (OSHB) shown in the  
   
 lexicon header on word click.  
**UI polish pass:**  
- Soft highlight palette (muted pastels at render time; legacy storage values  
   
 unchanged for data compat).  
- Drop-cap on verse 1; muted chapter heading at top of pane.  
- Curated serif stack with warm off-white text color in dark mode.  
- Hover-only Strong's underlines (no permanent wall of underlines).  
- Slim single-row cross-ref bar.  
- Combined Book + Chapter popover (Apple-Books style).  
- F11 distraction-free reading mode.  
- AdwToastOverlay for transient feedback.  
- Empty states in Study Journal, Module Manager, Search panel.  
- Note editor lives in Adw.Window (not popover) to avoid Wayland  
   
 popover-inside-popover lifecycle issues.  
- Lexicon panel restyle: accent top border, badge-style word-study header,  
   
 subtle left border on word-study split.  
**Stability hardening (multiple passes):**  
- Threading audit (RLock, dedicated indexing lock, atomic spawn check).  
- _NullSWMgr fallback when SWORD lib fails to initialize.  
- File hardening: UTF-8 + ensure_ascii=False on all JSON, isinstance type  
   
 checks, WAL mode for eBible SQLite.  
- Popover lifetime: all set_parent-mode popovers unparent on closed.  
- Module change clears stale per-module state (Strong's, morph, lexicon,  
   
 dict popup).  
- Sync auto-unlocks on devotional → Bible module switch.  
- Flash highlight: independent timers per flash, priority bumped each apply,  
   
 150ms defer after scroll, _cancel_all_flashes before buffer reset.  
- **In-place annotation tag application** — right-click highlight / underline /  
   
 note save no longer triggers a chapter re-render. Zero scroll movement.  
**Pre-Flatpak cleanup progress**  
Phases A–E done; Phase F (Flathub PR) is the remaining gate. Details  
   
 below.  
**Phase A — Dead-code sweep (done):** Removed _restore_scroll,  
   
 _find_topmost_visible_offset, _scroll_to_offset_silent, the legacy  
   
 _saved_* fields, the unused keep_scroll parameter on _fetch_and_render,  
   
 the unused annotation parameter on _html_to_markup, and the unreachable  
   
 _on_book_changed / _on_chapter_changed handlers (dropdowns are hidden;  
   
 the combined Book+Chapter popover drives all navigation through _go_to).  
   
 −102 / +28 lines.  
**Phase B — Tests for the bridges (done):** 113 pytest tests across four  
   
 files covering sword_bridge, open_data, annotations, reading_plans.  
   
 Found and fixed one real bug along the way (annotations.save_tags  
   
 crashed on None entries — used t.strip() on raw values after a defensive  
   
 str(t).strip() filter). Runs in <0.5s.  
**Phase C — Split ** **pane.py** ** (done):** Three extractions:  
- devotional.py (C.1) — render_osis(buffer, raw, dark) +  
 _insert_ref(...). Pure buffer manipulation.  
- annotation_dialogs.py (C.2) — study menu, note editor (Adw.Window),  
   
 chapter note popover, compare translations, copy verse, suggested topics.  
   
 Module-level functions taking pane as first parameter.  
- lexicon_panel.py (C.3) — LexiconPanel(Gtk.Box) class. Owns its own  
   
 widgets, history, current Strong's/morph state. BiblePane composes it.  
pane.py shrank from 2118 → 1338 lines (-37%). All flows verified by user.  
...
- .gitignore updated to track data/icons/ and data/*.desktop while  
   
 still excluding the downloaded open-data files (cross_references.txt,  
   
...
- Accessibility audit, i18n setup — deferred until strings stabilize.  
- Architectural review by an experienced GTK4 developer — optional but  
   
 recommended.  
**Session expansion (2026-05-20) — UX + perf**  
Wide pass added since the original release-polish commit. Highlights  
   
 (architectural facts only — implementations live in the current code):  
**Reading column + scrollbar** — dynamic margins via  
   
 _ReadingScrolledWindow subclass; scrollbar on outer pane edge; Width  
   
 slider in Text Appearance card (settings.reading_width, 540–1600 px).  
**Per-pane Copy Chapter button** moved from menu panel to each pane  
   
 toolbar.  
**Study Journal master-detail rewrite** with live-save color/underline,  
   
 explicit-save note/tags, free-text SearchEntry, clickable tag chips,  
   
 and a TagManagerWindow with rename (auto-merging)/delete.  
   
 annotations.get_tag_counts(), rename_tag(), delete_tag().  
**Module picker rewrite** — MenuButton + popover with a Gtk.Stack  
   
 list page (search + lang chips + module list with ⓘ buttons) and info  
   
 page (description / language / version / type / copyright / license /  
   
 about). sword_bridge.module_language() + module_info();  
   
 ebible_bridge.module_language() + module_info(). Class-level  
   
 _module_lang_cache memoizes.  
**Book/Chapter popover → verse picker** — right-click a chapter to  
   
 slide over to a 5-wide verse FlowBox via Gtk.Stack(SLIDE_LEFT_RIGHT).  
   
 sword_bridge.verse_count() added.  
**Reading-mode exit affordance** — circular X button in a SLIDE_DOWN  
   
 revealer at top-center, revealed after 2s hover in the top 12 px zone,  
   
 hidden when cursor exits the 80 px keep-zone. Motion controller on the  
   
 window (not the overlay) so events reach it regardless of cursor  
   
 position.  
**Selection + clipboard**  
- Drag-select sticks (release handler distinguishes click vs drag by  
   
 press-vs-release coord delta > 4 px).  
- Ctrl+C prepends Book Ch:V[-V2] (Module) for selections anchored to  
   
 verses; view.connect('copy-clipboard', ...) +  
 view.stop_emission_by_name('copy-clipboard').  
**Navigation + zoom keybinds**  
- Key controller phase = CAPTURE (was BUBBLE) — fixes "keybinds stop  
   
 working after a while".  
- Mouse wheel over the title button cycles chapters.  
- Home/End = first/last verse, gated on _focus_is_text_input().  
- Ctrl+1 / Ctrl+2 / Ctrl+Tab for pane focus.  
- Ctrl+scroll / pinch zoom font size via BiblePane(on_font_size_request=…).  
**Search polish**  
- F3 / Shift+F3 step through results in either surface (window panel  
   
 or per-pane search). Falls back to re-revealing a closed panel if  
   
 results are still cached.  
- Match-case toggle (Aa) in both surfaces — Whoosh path post-filters  
   
 results; eBible path uses GLOB with *…* wildcards.  
- Matched-word highlight on arrival from search — _search_hl tag,  
   
 amber #ffd180 (distinct from flash yellow), 5 s auto-expire.  
 _pending_search_highlight stashed by the originating surface  
   
 before navigation; consumed by _apply_search_highlight at end of  
 _fetch_and_render. Cleared on module change and at every render  
   
 reset path.  
**Dictionary popup polish**  
- Stack switcher tabs get margin_start/end=18 to stay off popup  
   
 edges.  
- _short_dict_title(mod_name, mod_desc) produces tab labels: explicit  
   
 map for common modules + heuristic fallback that picks first 1-2  
   
 distinctive words + 4-digit year, stopping at fluff (Dictionary,  
   
 Encyclopedia, Revised, Unabridged, etc.).  
- _html_to_markup now handles <br>, </p>, </li>, <b> for  
   
 long-form dicts (Webster's 1913). OSIS commentaries don't use these,  
   
 so existing render paths unaffected.  
**Compositor audit + Hyprland support**  
- welcome.pyModuleManagerWindow passes transient_for=self, modal=True.  
- README has "Running on tiling compositors" section with  
 windowrulev2 = float, … examples + the xdg-desktop-portal-gtk  
   
 note.  
**Session state**  
- pane1_top_verse / pane2_top_verse settings keys persist per-pane  
   
 scroll. Restored via _restore_top_verse (existing field, also used  
   
 for lexicon-toggle reflow) — _scroll_to_verse_silent (no flash) on  
   
 initial load.  
- recent_passages setting (capped 10, deduped); header button +  
   
 popover; trash icon clears.  
**Dark-mode jump bar** — .jump-bar CSS class with @view_bg_color  
   
 override (the .card default is semi-transparent in dark themes).  
**Performance audit (5 fixes, all shipped)**  
- **eBible thread-local SQLite connection.**_conn_local = threading.local() + _db() caches. Removed all conn.close() call  
   
 sites. Hot paths (load_chapter, search_module, module_language,  
   
 module_info) no longer pay open/PRAGMA/CREATE/COMMIT churn per call.  
- **_module_lang** ** memoization** — class-level dict[str, str] shared  
   
 across panes. Invalidated in refresh_modules() so installs/removals  
   
 don't keep stale entries.  
- **SearchPanel display cap = 500.**_DISPLAY_CAP = 500. F3 still  
   
 walks the full _results (up to 5000). Footer hint row appended  
   
 when total > 500.  
- **settings.put** ** debounce.**threading.Timer with  
 _SAVE_DEBOUNCE_S = 0.5 under _save_lock. New settings.flush()  
   
 called from close-request after final puts.  
- **Tag-table bounding.**_clear_chapter_scoped_tags() called per  
   
 render via every _display* path right after set_text(''). Prefix  
   
 list: vnum_, strg:, morph:, phrase:, devref:. Bounded tags  
   
 (hl_*, _flash, _search_hl, _ul_text, _note_marker,  
 _strg_hover) stay.  
**Hotkeys reference window** (menu panel) up to date with all new  
   
 shortcuts — Navigation / Panes & view / Selection sections.  
**Test count: 113 → 124** (11 new annotation tag-management tests).  
**Discussed / not yet decided**  
- Persistent "current verse" indicator (thin left-edge bar on the selected  
   
 verse, distinct from the 1-second flash).  
- Pane-swap button.  
- Focus-pane toggle (Ctrl+\ to temporarily hide the other pane).  
- Footnote support (translator notes from SWORD <note> elements).  
- Topical tag browser as a top-level surface.  
- Bible reference URI scheme (bible:John+3:16).  
- Print current chapter (Ctrl+P).  
- Triple-click verse select — implemented + removed (conflicted with  
   
 double-click dict popup). Drag-select replaces it.  
