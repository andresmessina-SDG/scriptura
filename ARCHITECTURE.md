...
actions; rename auto-merges. `annotations.get_tag_counts/rename_tag/
delete_tag` back it.
...
- `welcome.py` `ModuleManagerWindow` passes `transient_for=self,
  modal=True`.
...
- **eBible thread-local SQLite connection.** `_conn_local =
  threading.local()` + `_db()` caches. Removed all `conn.close()` call
