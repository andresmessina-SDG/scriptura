...
from pane_peek import PeekController
...
class BiblePane(Gtk.Box):
    ...
    def __init__(self, module_name=None, on_word_click=None,
        ...
        # The double-click dictionary, footnote and verse peeks (pane_peek.py).
        self._peek = PeekController(self)
        ...
        gesture_dict.connect('pressed', self._peek.on_dict_click)
    ...
    def _on_left_click(self, gesture, n_press, x, y):
        ...
        if fnote:
            ...
            self._peek.show_footnote_peek(fnote, it)
            return
    ...
    # ── Peek delegators (pane_peek.py) ───────────────────────────────────

    def dismiss_dict_peek(self):
        """Close an open peek; True if one was open (the window's Escape)."""
        return self._peek.dismiss()

    def show_anchored_peek(self, anchor_widget, rect, content):
        self._peek.show_anchored(anchor_widget, rect, content)

    def attach_dict_to_label(self, label):
        """Double-click look-up on a card's label (commentary, caption)."""
        self._peek.attach_to_label(label)
