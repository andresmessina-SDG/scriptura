...
from appearance_page import AppearancePageMixin
...
class BibleWindow(AppearancePageMixin, Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        # Overlay lifecycle (menu / search / jump mutual-exclusion + toggles);
        # the window keeps thin delegates. Lazy import for the same reason.
        from overlays import OverlayManager
        self._overlays = OverlayManager(self)
        # Presentation mode (fullscreen large-type projection, built on reading
        # mode); the window keeps thin delegates + a forwarding _present_mode.
        from present_mode import PresentController
        self._present = PresentController(self)
    ...
    # ── Overlays (delegated to OverlayManager; see overlays.py) ──────────────
    def _show_jump(self):
        self._overlays._show_jump()

    def _hide_jump(self):
        self._overlays._hide_jump()

    def _on_jump_activate(self, entry):
        self._overlays._on_jump_activate(entry)

    def _parse_jump(self, text):
        return self._overlays._parse_jump(text)

    def _ensure_menu_panel(self):
        return self._overlays._ensure_menu_panel()

    def _toggle_menu(self, btn):
        self._overlays._toggle_menu(btn)

    def _on_search_clicked(self, btn):
        self._overlays._on_search_clicked(btn)
    ...
    # ── Presentation mode (delegated to PresentController; present_mode.py) ───
    @property
    def _present_mode(self):
        return self._present._present_mode

    def _toggle_present_mode(self):
        self._present._toggle_present_mode()

    def _set_present_mode(self, on):
        self._present._set_present_mode(on)

    def _present_cross(self, delta):
        self._present._present_cross(delta)

    def _present_jump(self, book, chapter, verse):
        self._present._present_jump(book, chapter, verse)

    def _build_present_controls(self, overlay):
        self._present._build_present_controls(overlay)

    def _sync_present_controls(self):
        self._present._sync_present_controls()

    def _present_toggle_parallel(self, on):
        self._present._present_toggle_parallel(on)
